...
function latestMetricAverage(metrics, zoneId, metricType) {
  ...
  const m = String(metricType ?? '').trim().toLowerCase()
  const rows = (metrics || []).filter(
    (row) =>
      String(row.zone_id ?? '').trim() === z &&
      String(row.metric_type ?? '').trim().toLowerCase() === m
  )
...
function DashboardWorkspaceSwitch({ value, onChange }) {
  return (
    <div className="dashboard-workspace-bar">
      <span className="dashboard-workspace-bar__label" id="workspace-switch-label">
        Workspace
      </span>
      <div
...
function OperatorAlertCard({ alert, actingAlertId, onAcknowledge, onResolve }) {
  return (
...
export default function AlertDashboardPresentation({
  ...
  const zoneFilterOptions = useMemo(() => {
    const s = new Set()
    alerts.forEach((a) => s.add(a.zone_id))
    return [...s].sort()
  }, [alerts])

  const metricFilterOptions = useMemo(() => {
    const s = new Set()
    alerts.forEach((a) => s.add(a.metric_type))
    return [...s].sort()
  }, [alerts])
  ...
  const filteredAlerts = useMemo(() => {
    return alerts.filter((a) => {
      ...
      if (alertMetricFilter && a.metric_type !== alertMetricFilter) return false
      return true
    })
  }, [alerts, alertZoneFilter, alertMetricFilter])
    ...
    <section className="page-shell alert-dashboard-page">
      {isAdmin && (
        <div className="toolbar-card dashboard-workspace-card">
          <DashboardWorkspaceSwitch value={adminWorkspace} onChange={setAdminWorkspace} />
        ...
        <div className="toolbar-card admin-rules-card">
          <div className="admin-rules-card__header">
            ...
            <p className="muted-copy admin-rules-card__sub">
              Loaded from <code>/admin/rules</code>. New rules apply on the next evaluation cycle.
            </p>
          </div>
                  ...
                  (rules || []).map((r) => (
                    <tr key={r.rule_id}>
                      ...
                      <td>
                        ...
                        >
                          {deletingRuleId === r.rule_id ? 'Deleting…' : 'Delete'}
                        </button>
                      </td>
                    </tr>
                  ))
          ...
          <div className="admin-rule-form-section">
            ...
            <form className="admin-rule-form" onSubmit={submitRule}>
              ...
              <div className="admin-rule-form__actions">
                <button type="submit" className="action-button" disabled={ruleSaving}>
                  {ruleSaving ? 'Saving…' : 'Create rule'}
                </button>
              </div>
            </form>
            {ruleFormError ? <p className="auth-error admin-rule-form__error">{ruleFormError}</p> : null}
          </div>
        </div>
      )}
      ...
      {showAdminWorkspace && (rules || []).length > 0 && (
        <div className="toolbar-card admin-histogram-card">
          <h3 className="admin-histogram-card__heading">Levels vs thresholds</h3>
          <p className="muted-copy admin-histogram-card__sub">
            Green = within threshold (no alert). Red = violation side. Marker = latest 5‑minute aggregate per
            zone/metric (<code>/admin/metrics/latest</code>).
          </p>
          <div className="admin-histogram-list">
            {(rules || []).map((rule) => (
              <AdminThresholdBar
                key={rule.rule_id}
                rule={rule}
                latestAverage={latestMetricAverage(metrics, rule.zone_id, rule.metric_type)}
              />
            ))}
          </div>
        </div>
      ...
      <div className="toolbar-card operator-alerts-map-panel">
        <div className="operator-alerts-panel__head">
          ...
          <p className="muted-copy operator-alerts-panel__lede">
            Use filters or click a zone on the map. The list scrolls (about three cards tall); the map is below.
          </p>
        </div>
        ...
        <div className="operator-alert-list-scroll">
          ...
          {alerts.length > 0 && filteredAlerts.length === 0 && (
            <p className="muted-copy operator-alerts-none-matching">No alerts match these filters.</p>
          )}
        ...
        <div className="operator-map-section">
          ...
          <p className="muted-copy operator-map-section__lede">
            Click a zone to filter the list to that zone. Open a pin to see all alerts for that zone (metric
